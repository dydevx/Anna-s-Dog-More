...
export function ProductCard({ product, locale, priority = false }: { product: Product; locale: Locale; priority?: boolean }) {
  ...
  const from = product.variants.length > 1 || product.variants.some((variant) => variant.price === null);
  ...
  return <article className="product-card">
    <Link className="product-image" href={`/${locale}/product/${product.slug}`}>
      ...
      {soldOut && <span className="product-badge muted-badge">{locale === "de" ? "Nicht verfügbar" : "Unavailable"}</span>}
    </Link>
